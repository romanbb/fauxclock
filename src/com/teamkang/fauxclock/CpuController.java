...
public class CpuController extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		if (intent.getAction() == Intent.ACTION_BOOT_COMPLETED
				&& settings.getBoolean("load_on_startup", false)) {

			CpuController cpu = new CpuController(context);
			cpu.readVddCpuTable();
			cpu.readGovs();

			cpu.setGov(settings.getString("cpu_gov", cpu.getCurrentActiveGov()));
			cpu.setMinFreq(settings.getString("cpu0_min_freq",
					cpu.getMinFreq(0)));
			cpu.setMaxFreq(settings.getString("cpu0_max_freq",
					cpu.getMaxFreq(0)));
